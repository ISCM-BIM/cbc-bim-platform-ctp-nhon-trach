  ...
  if (OPTIONS.length === 0) {
    return (
      <div className="panel p-4">
        <p className="mb-3 text-sm font-semibold text-on-surface">So sánh trước / sau thay đổi</p>
        <div className="flex flex-col items-center justify-center gap-2 py-8 text-center">
          <ImageOff size={22} className="text-outline" />
          <p className="text-xs text-on-surface-variant">Chưa có thay đổi hiện trường nào để so sánh.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="panel p-4">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm font-semibold text-on-surface">So sánh trước / sau thay đổi</p>
        <select
          value={selectedId}
