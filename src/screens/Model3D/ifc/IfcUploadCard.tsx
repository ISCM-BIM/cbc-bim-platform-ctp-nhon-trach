...
const STAGE_LABEL_EN: Record<IfcParseProgress['stage'], string> = {
  fetching: 'Downloading model file...',
  reading: 'Reading file...',
  opening: 'Opening IFC model...',
  structure: 'Reading spatial tree...',
    ...
    <div className="flex h-full flex-col items-center justify-center gap-4 p-8 text-center">
      {status === 'error' ? (
        <>
          ...
          <div>
            <p className="font-heading text-base font-semibold text-on-surface">{tr('Không tải được mô hình dự án', 'Could not load the project model')}</p>
        ...
        <>
          ...
          <p className="text-sm font-medium text-on-surface">
            {progress ? (language === 'en' ? STAGE_LABEL_EN[progress.stage] : STAGE_LABEL[progress.stage]) : tr('Đang tải mô hình dự án...', 'Loading project model...')}
          </p>
          ...
          <p className="max-w-xs text-[11px] text-outline">
            {tr(
              ...
              'The model is parsed entirely in your browser and never uploaded to any server.',
            )}
          </p>
          {sizeWarning && (
