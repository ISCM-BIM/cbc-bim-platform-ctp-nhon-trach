...
const IMAGE_LABELS_EN = ['3D clash location', 'Plan view of clash location']

export function ModelClashPanel({ clash, index, total, onClose }: ModelClashPanelProps) {
  const { language, tr } = useLanguage()
  const imageLabels = language === 'en' ? IMAGE_LABELS_EN : IMAGE_LABELS
      ...
      <div className="panel-strong absolute right-4 top-4 max-h-[calc(100%-2rem)] w-80 overflow-y-auto p-4">
        <div className="mb-3 flex items-start justify-between gap-2">
          <p className="label-caps flex items-center gap-1.5 text-brand">
            <AlertTriangle size={13} /> {tr('Va chạm đã kiểm tra', 'Clash checked')} ({index + 1}/{total})
          </p>
          ...
          {clash.images.map((img, i) => (
            <div key={i}>
              <button
                ...
                title={tr('Bấm để phóng to', 'Click to zoom in')}
              >
                <img src={img} alt={`${clash.title} - ${imageLabels[i] ?? i + 1}`} className="w-full" />
                <span className="absolute inset-0 flex items-center justify-center bg-black/0 opacity-0 transition-all group-hover:bg-black/30 group-hover:opacity-100">
                  <ZoomIn size={22} className="text-white drop-shadow" />
                </span>
              </button>
              {imageLabels[i] && <p className="mt-1 text-[10px] text-on-surface-variant">{imageLabels[i]}</p>}
            </div>
          ))}
        </div>
      </div>
